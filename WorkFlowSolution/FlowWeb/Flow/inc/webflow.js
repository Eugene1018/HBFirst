...
function editFlow() {
    ...
    var result = workFlowDialog(Flow.getElementsByTagName("BaseProperties").item(0).getAttribute("flowId"), 'Flow');
    if (result == "ok" && confirm("确认要保存吗？")) {
...
function exportFlow() {
    ...
    web.document.write('<?xml version="1.0" encoding="GBK?"?>' + document.getElementById("FlowXML").value);
    web.document.execCommand('SaveAs', false, 'WorkFlowXml.xml')
}
function saveBtn() {
    if (document.all.FlowXML.value == '') {
        alert('流程信息为空，不能执行此操作！');
...
function objFocusedOff() {
    ...
    if (FlowXML.value != '') {
        ...
        if (FOCUSEDOBJID.lastIndexOf('$txt') > 0) {
            focusedOffColor = Flow.getElementsByTagName("VMLProperties").item(0).getAttribute("actionTextColor");
        }
...
function redrawVML() {
    ...
    if (FlowXML.value != '') {
        ...
        var ActionTextColor = Flow.getElementsByTagName("VMLProperties").item(0).getAttribute("actionTextColor");
        ...
        for (var i = 0; i < Steps.childNodes.length; i++) {
            ...
            nColor1 = StepColor1;
            nColor2 = StepColor2;
            var spColor = Step.getElementsByTagName("VMLProperties").item(0).getAttribute("stepColor1");
            if (spColor != null && spColor != '') {
                nColor1 = spColor;
            }
            nIs3D = IsStep3D;
        ...
        for (var i = 0; i < Actions.childNodes.length; i++) {
            ...
            lActionTextColor = ActionTextColor;
            result = getActionHTML(lType, lIndex, lId, lText, document.getElementById(lFromStep), document.getElementById(lToStep), lStrokeColor, lStrokeWeight, lStartArrow, lEndArrow, lActionTextColor)
...
function getActionHTML(actionType, z_index, id, actionText, fromStep, toStep, strokeColor, strokeWeight, startArrow, endArrow, lActionTextColor) {
    var actionHTML = '';

    if (actionType == 'Line') {
        actionHTML = '<v:line id=' + id + ' title="' + actionText + '" style="z-index:' + z_index + ';position:absolute;" ' + getActionPoints(actionType, fromStep, toStep) + ' strokecolor="' + strokeColor + '" strokeweight="' + strokeWeight + '" onmousedown=\'cleancontextMenu();objFocusedOn(this.id,"Action");\' oncontextmenu=\'objFocusedOn(this.id,"Action");actionContextMenu("' + id + '","' + actionText + '");return false;\'><v:stroke StartArrow="' + startArrow + '" EndArrow="' + endArrow + '"/></v:line>';
    } else {
        actionHTML = '<v:PolyLine id=' + id + ' title="' + actionText + '" filled="false" Points="' + getActionPoints(actionType, fromStep, toStep) + '" style="z-index:' + z_index + ';position:absolute;" strokecolor="' + strokeColor + '" strokeweight="' + strokeWeight + '" onmousedown=\'cleancontextMenu();objFocusedOn(this.id,"Action");\' oncontextmenu=\'objFocusedOn(this.id,"Action");actionContextMenu("' + id + '","' + actionText + '");return false;\'><v:stroke StartArrow="' + startArrow + '" EndArrow="' + endArrow + '"/></v:PolyLine>';
        actionHTML += '<v:PolyLine id=' + id + "$txt" + ' title="' + actionText + '" filled="false" Points="' + getTextActionPoints(fromStep, toStep) + '" style="z-index:' + z_index + ';position:absolute;" strokecolor="' + lActionTextColor + '" strokeweight="' + strokeWeight + '" onmousedown=\'cleancontextMenu();objFocusedOn(this.id,"Action");\' oncontextmenu=\'objFocusedOn(this.id,"Action");actionContextMenu("' + id + '");return false;\'><v:stroke StartArrow="' + startArrow + '" EndArrow="' + endArrow + '"/><v:path textpathok="t"/><v:textpath id="txt" style="font-size:12px;" on="t" fitpath="f" string=' + actionText + ' /></v:PolyLine>';
    }
...
function getTextActionPoints(fromStep, toStep) {
    if (fromStep == null || toStep == null) return '';
    ...
    var strs = pointsHTML.split(' ');
    if ((strs[0].split(',')[0] == strs[1].split(',')[0]) && (strs[1].split(',')[0] == strs[2].split(',')[0])) {
        pointsHTML = strs[0] + ' ' + (parseInt(strs[1].split(',')[0]) + 1) + "," + strs[1].split(',')[1] + ' ' + strs[2];
    } else if ((strs[0].split(',')[1] == strs[1].split(',')[1]) && (strs[1].split(',')[1] == strs[2].split(',')[1])) {
        pointsHTML = strs[0] + ' ' + strs[1].split(',')[0] + ',' + (parseInt(strs[1].split(',')[1]) + 1) + ' ' + strs[2];
    }
    return pointsHTML;
}
...
function flowContextMenu() {
    //菜单源文本
    var menuSrc = '<menu><entity id="c0"><description>添加新步骤</description><image>inc/contextMenu/images/add_step.gif</image><fontcolor>' + MenuTextColor_enable + '</fontcolor><onClick>cleancontextMenu();newStep("NormalStep");</onClick><contents></contents></entity>';
    menuSrc += '<entity id="c1"><description>添加新动作</description><image>inc/contextMenu/images/add_action.gif</image><fontcolor>' + MenuTextColor_enable + '</fontcolor><onClick>cleancontextMenu();newAction();</onClick><contents></contents></entity>';
    menuSrc += '<entity id="c2"><description>局部刷新</description><image>inc/contextMenu/images/refresh_vml.gif</image><fontcolor>' + MenuTextColor_enable + '</fontcolor><onClick>cleancontextMenu();redrawVML();</onClick><contents></contents></entity></menu>';
    showContextMenu(menuSrc);
}

function stepContextMenu(stepId, stepType, text) {
    //菜单源文本
    var menuSrc = '<menu><entity id="c0"><description>编辑步骤[' + text + ']</description><image>inc/contextMenu/images/edit_obj.gif</image><fontcolor>' + MenuTextColor_enable + '</fontcolor><onClick>cleancontextMenu();editStep("' + stepId + '","' + stepType + '");</onClick><contents></contents></entity>';
    if (stepType != 'BeginStep' && stepType != 'EndStep') menuSrc += '<entity id="c1"><description>删除步骤[' + text + ']</description><image>inc/contextMenu/images/del_obj.gif</image><fontcolor>' + MenuTextColor_enable + '</fontcolor><onClick>cleancontextMenu();delStep("' + stepId + '");</onClick><contents></contents></entity>';
    menuSrc += '<entity id="c2"><description>局部刷新</description><image>inc/contextMenu/images/refresh_vml.gif</image><fontcolor>' + MenuTextColor_enable + '</fontcolor><onClick>cleancontextMenu();redrawVML();</onClick><contents></contents></entity></menu>';
    showContextMenu(menuSrc);
}

function actionContextMenu(actionId, actionText) {
    //菜单源文本
    var menuSrc = '<menu><entity id="c0"><description>编辑动作[' + actionText + ']</description><image>inc/contextMenu/images/edit_obj.gif</image><fontcolor>' + MenuTextColor_enable + '</fontcolor><onClick>cleancontextMenu();editAction("' + actionId + '");</onClick><contents></contents></entity>';
    menuSrc += '<entity id="c1"><description>删除动作[' + actionText + ']</description><image>inc/contextMenu/images/del_obj.gif</image><fontcolor>' + MenuTextColor_enable + '</fontcolor><onClick>cleancontextMenu();delAction("' + actionId + '");</onClick><contents></contents></entity>';
    menuSrc += '<entity id="c2"><description>局部刷新</description><image>inc/contextMenu/images/refresh_vml.gif</image><fontcolor>' + MenuTextColor_enable + '</fontcolor><onClick>cleancontextMenu();redrawVML();</onClick><contents></contents></entity></menu>';
    showContextMenu(menuSrc);
}
...
function workFlowDialog(id, type) {
    ...
    switch (type) {
        case 'NormalStep': dialogURL = id == null ? 'StepDialog.aspx' : 'StepDialog.aspx?stepid=' + id; break;
        case 'BeginStep': dialogURL = id == null ? 'StepDialog.aspx' : 'StepDialog.aspx?stepid=' + id; break;
        case 'EndStep': dialogURL = id == null ? 'StepDialog.aspx' : 'StepDialog.aspx?stepid=' + id; break;
        case 'Action': dialogURL = id == null ? 'ActionDialog.aspx' : 'ActionDialog.aspx?actionid=' + id; break;
        case 'Flow': dialogURL = id == null ? 'FlowDialog.aspx' : 'FlowDialog.aspx?flowid=' + id; break;
    }
