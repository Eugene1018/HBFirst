...
function moveIt() {
    ...
    distanceX = parseInt(XRATE*(newX - CURRENTX));//X×ø±ê»»Ëã
    distanceY = parseInt(YRATE*(newY - CURRENTY));//Y×ø±ê»»Ëã	
	    ...
	    if(FlowXML.value!=''){
		   ...
		   for ( var i = 0;i < Actions.childNodes.length;i++ ) {
               ...
               if(fromStep == MOVEOBJ.id || toStep == MOVEOBJ.id){                  
                  ...
                  document.getElementById(aId + "$txt").points.value = getTextActionPoints(document.getElementById(fromStep), document.getElementById(toStep));				  
               }
